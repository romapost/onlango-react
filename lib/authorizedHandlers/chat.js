...
export default {
  ...
  async [joinRoom](id, socket) {
    ...
    if (room) {
      ...
      socket.join(roomId);
      socket.to(roomId).broadcast.emit('dispatch', changeChatStatus({id: userId, status: 'online'}));
      return joinRoom({
        lastMessages,
        usersOnline: (room.usersOnline || []).filter(e => e.toString() != userId.toString()),
  ...
  async [changeChatStatus](status, socket) {
    const {userId: id} = socket;
    switch (status) {
      case 'idle':
      case 'typing':
        socket.broadcast.emit('dispatch', changeChatStatus({id, status}));
        break;
    }
  }
};
