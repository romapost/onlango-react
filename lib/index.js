...
logger.format('log', function developmentFormatLine(tokens, req, res) {
  ...
  if (!fn) fn = developmentFormatLine[color] = logger.compile(
    `\x1b[0m:time :ip \x1b[${color}m:status \x1b[0m:method :url :response-time ms - :res[content-length]\x1b[0m`
  );
...
if (!config.dev) {
  app.use(compression());
  app.use((req, res, next) => { res.setHeader('Cache-Control', 'public, max-age=86400000'); next() });
}
...
app.use('/fonts', express.static(path.resolve(__dirname, '../node_modules/font-awesome/fonts')));
