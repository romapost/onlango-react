...
logger.token('time', () => { const d = new Date(); return `${d.toLocaleDateString()} ${d.toLocaleTimeString()}` });
logger.format('log', function developmentFormatLine(tokens, req, res) {
  ...
  if (!fn) fn = developmentFormatLine[color] = logger.compile(
    `\x1b[0m:time :remote-addr \x1b[${color}m:status \x1b[0m:method :url :response-time ms - :res[content-length]\x1b[0m`
  );
