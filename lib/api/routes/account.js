      ...
      image
        ...
        .toBuffer('jpg', (err, buf) => {
          ...
          const hash = req.userPicHash = crypto
            ...
            .update(buf)
            .digest('hex');
          const path = `public/uploads/userpic/${hash}`;
  ...
  .post((req, res, next) => {
    Account.findOne({email: req.user.id}, (err, account) => {
      if (err) next(err);
        ...
        account.teacherForm = {...account.teacherForm, ...req.body};
    ...
    Account.findOne(
      ...
      (err, account) => {
        if (err) next(err);
          ...
          const data = account.teacherForm || {};
          if ('resumeFile' in data) delete data.resumeFile;
          res.json(data);
        }
      }
    );
...
const uploadResumeFiles = multer({
  dest: 'public/uploads/teacherResumeFiles/',
  limits: {fileSize: 5 * 1024 * 1024}
});
...
router.post(
  ...
  checkAuth,
  uploadResumeFiles.single('resumeFile'),
  (req, res, next) => {
    Account.findOneAndUpdate(
        {email: req.user.id},
        {'teacherForm.resumeFile': req.file.filename},
        {fields: {'teacherForm.resumeFile': 1}},
        (err, account) => {
          const {resumeFile} = account.teacherForm;
          if (resumeFile) fse.remove(`public/uploads/teacherResumeFiles/${resumeFile}`);
          if (err) next(err);
          else res.end();
        }
      );
...
router.get('/teacherslist', (req, res, next) => {
  Account.find({status: 'teacher'}, (err, list) => {
    if (err) next(err);
    else res.json(list.map(e => {
      const {_id: teacherId, name, image, userinfo: {country}, teacherForm: {experience, languages}} = e;
      return {image, name, experience, country, languages, teacherId};
    }));
  });
});

router.get('/teacherinfo/:teacherId', (req, res, next) => {
  console.log(req.params.teacherId);
  Account.findOne({_id: req.params.teacherId}, (err, account) => {
    if (err) next(err);
