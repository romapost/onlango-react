  ...
  .get((req, res, next) => {
    Account.findOne({email: req.user.id}, (err, doc) => {
      if (err) next(err);
        ...
        const {email, name, surname, phone, gender, country, city, birthdate, image} = doc;
        res.json({email, name, surname, phone, gender, country, city, birthdate, image});
      }
    });
  })
  .post((req, res, next) => {
    const data = {};
    for (let k of ['email', 'name', 'surname', 'phone', 'gender', 'country', 'city', 'birthdate', 'image'])
      if (req.body[k]) data[k] = req.body[k];
    Account.findOneAndUpdate(
      {email: req.user.id},
      data,
      {new: true},
