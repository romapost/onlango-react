...
export default ({addEmoji, visible, show}) => <Col>
  {
    visible
      ? <div style={{display: 'inline-block', borderRadius: 3, border: '1px solid #aab', backgroundColor: '#ddf'}}>{list.map(e => <span style={{display: 'inline-block', margin: '0.7rem 0.2rem 0'}} onClick={() => addEmoji(e)}>{emojify(e, emojiOptions)}</span>).concat(<button style={{float: 'right', margin: '0.5rem'}} onClick={() => { addEmoji('') }}>X</button>)}</div>
